...
import { saveDraft, submitProposal, getEligibleCoStudents, getMyProposals, getProposalById, uploadProposalPdf } from '../services/proposalService';
...
const ProposeProjectPage = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    endOfStudies: user?.endOfStudies ? new Date(user.endOfStudies).toISOString().substring(0, 10) : '',
    coStudent: '',
    suggestedMentor: '',
    attachmentName: '',
    attachmentId: '',
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const editableProposal = myProposalsRes.data.find(p => p.status === 'Draft' || p.status === 'Rejected');

        if (editableProposal) {
          setProposal(editableProposal);
          setFormData({
            projectName: editableProposal.projectName || '',
            background: editableProposal.background || '',
            objectives: editableProposal.objectives || '',
            marketReview: editableProposal.marketReview || '',
            newOrImproved: editableProposal.newOrImproved || '',
            address: editableProposal.authorSnapshot?.address || user?.address || '',
            mobilePhone: editableProposal.authorSnapshot?.mobilePhone || user?.mobilePhone || '',
            endOfStudies: (editableProposal.authorSnapshot?.endOfStudies || user?.endOfStudies)
              ? new Date((editableProposal.authorSnapshot?.endOfStudies ?? user?.endOfStudies) || '')
              : '',
            coStudent: editableProposal.coStudent?._id || '',
            suggestedMentor: editableProposal.suggestedMentor?._id || '',
            attachmentName: editableProposal.attachments?.filename || '',
            attachmentId: editableProposal.attachments?.fileId || '',            
          });
        }
      } catch (e) {
        console.error(e);
      ...
      <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 2 }}>
        <Stack direction="row" spacing={1} alignItems="center">
          ...
          <Typography variant="h4">
            {proposalId ? 'Edit Project Proposal' : 'Propose a New Project'}
          </Typography>
        </Stack>
        {isSaving && (
          <Stack direction="row" spacing={1} alignItems="center">
        ...
        <SectionCard title="Student Details" step={2}>
          <Grid container spacing={2}>
            <Grid item size={{ xs: 12, sm: 6}}>
              <TextField fullWidth label="Student Name" value={user?.fullName || ''} InputProps={{ readOnly: true }} />
            </Grid>
            <Grid item size={{ xs: 12, sm: 6}}>
              <TextField fullWidth label="ID Number" value={user?.idNumber || ''} InputProps={{ readOnly: true }} />
            </Grid>
            <Grid item size={{ xs: 12}}>
              <TextField
                fullWidth
          ...
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} alignItems="center">
            ...
            {formData.attachmentName ? (
              <Chip
                ...
                variant="outlined"
                onDelete={formData.attachmentId ? () => setFormData(prev => ({ ...prev, attachmentId: '', attachmentName: '' })) : undefined}
                onClick={() => window.open(`${SERVER_URL}api/proposals/file/${formData.attachmentId}`, '_blank')}
              />
            ) : (
