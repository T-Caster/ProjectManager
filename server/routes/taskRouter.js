...
router.get("/mine", authMiddleware, async (req, res) => {
  try {
    ...
    if (!projects.length && user.role !== "hod") return res.json([]);

    const projectIds = projects.map((p) => p._id);
    let tasks = [];
    if (user.role !== "hod") {
      tasks = await populateTask(Task.find({ project: { $in: projectIds } }).sort({ dueDate: 1, createdAt: -1 }));
    } else {
      tasks = await populateTask(Task.find());
    }
 ...
 */
router.post("/", authMiddleware, roleMiddleware(["mentor"]), async (req, res) => {
  try {
    ...
    const meeting = await Meeting.findById(meetingId).populate("project", "name students mentor").populate("mentor", "_id");
...
router.get("/project/:projectId", authMiddleware, async (req, res) => {
  try {
    ...
    const tasks = await populateTask(Task.find({ project: project._id }).sort({ dueDate: 1, createdAt: -1 }));
    return res.json(tasks);
  } catch (err) {
    console.error(err);
    return res.status(500).json({ message: "Server error", error: err });
...
router.get("/meeting/:meetingId", authMiddleware, async (req, res) => {
  try {
    ...
    const tasks = await populateTask(Task.find({ meeting: meeting._id }).sort({ dueDate: 1, createdAt: -1 }));
    return res.json(tasks);
  } catch (err) {
    console.error(err);
    return res.status(500).json({ message: "Server error", error: err });
 ...
 */
router.put("/:taskId/complete", authMiddleware, roleMiddleware(["student", "mentor"]), async (req, res) => {
  try {
 ...
 */
router.put("/:taskId/reopen", authMiddleware, roleMiddleware(["mentor"]), async (req, res) => {
  try {
    const task = await Task.findById(req.params.taskId);
    if (!task) return res.status(404).json({ message: "Task not found" });
 ...
 */
router.put("/:taskId", authMiddleware, roleMiddleware(["mentor"]), async (req, res) => {
  try {
 ...
 */
router.delete("/:taskId", authMiddleware, roleMiddleware(["mentor"]), async (req, res) => {
  try {
    const task = await Task.findById(req.params.taskId);
    if (!task) return res.status(404).json({ message: "Task not found" });
